...
#  config.hostmanager.include_offline = true
  ...
  config.vm.define "master1" do |master1|
    ...
    # if origin_os == 'fedora'
    #   # Python2 is required by Ansible
  ...
  config.vm.define "admin1" do |admin1|
    ...
    ansible_groups = {
      ...
      'OSEv3:vars': {
        ...
        rhsm_pool: rhsm_pool,
        excluded_packages: "['docker*1.20*', 'docker*1.19*', 'docker*1.18*', 'docker*1.17*', 'docker*1.16*', 'docker*1.15*', 'docker*1.14*', 'docker*1.13*', 'docker*1.12*', 'docker*1.11*']"
      },
      etcd: ["master1"],
      nfs: ["admin1"],
      masters: ["master1"],
    ...
    # if origin_os == 'fedora'
    #   ansible_groups[:'OSEv3:vars'][:ansible_python_interpreter] = "/usr/bin/python3"
    # end
    ...
    ansible_host_vars = {
      ...
      node1: {
        ...
        openshift_node_labels: "\"{'region': 'primary', 'zone': 'east', 'tier': 'app'}\"",
        openshift_schedulable: true,
      ...
      node2: {
        ...
        openshift_node_labels: "\"{'region': 'primary', 'zone': 'west', 'tier': 'egress'}\"",
        openshift_schedulable: true,
    ...
    #     sudo dnf install -y python2-dnf libselinux-python
    #   SHELL
    # end
    ...
    admin1.vm.provision :ansible_local do |ansible|
      ...
      ansible.provisioning_path = '/home/vagrant/sync/egress_scalability'
      ansible.playbook       = '/home/vagrant/sync/egress_scalability/install.yaml'
